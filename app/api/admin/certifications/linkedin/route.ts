...
function parseGeminiJson(value: string) {
  ...
  const parsed = JSON.parse(cleaned) as { certifications?: ImportedCertification[] } | ImportedCertification[];
...
export async function POST(request: Request) {
  ...
  if (!validProfileCertificationsUrl(linkedInUrl)) {
    return NextResponse.json(
      { error: "Usa una URL de LinkedIn con formato /in/usuario/details/certifications/." },
      { status: 400 }
    );
  }
  ...
  if (!apiKey) {
    return NextResponse.json({ error: "AI_AGENT_API_KEY no está configurada." }, { status: 503 });
  }
  ...
  try {
    ...
    if (!linkedInResponse.ok) {
      throw new Error(`LinkedIn bloqueó la lectura pública (estado ${linkedInResponse.status}).`);
    }

    const html = await linkedInResponse.text();
    if (/authwall|checkpoint\/challenge|sign in to view/i.test(html)) {
      throw new Error("LinkedIn solicitó iniciar sesión y no expuso las certificaciones públicamente.");
    }
    ...
    if (!geminiResponse.ok) throw new Error(data.error?.message || "Gemini no pudo analizar la página.");
    ...
    if (certifications.length === 0) {
      throw new Error("No se encontraron certificaciones visibles en la página pública.");
    }

    return NextResponse.json({ certifications });
  } catch (error) {
    const message = error instanceof Error ? error.message : "No se pudieron importar las certificaciones.";
